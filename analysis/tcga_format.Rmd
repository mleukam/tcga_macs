...
All of this is recorded for completeness - this notebook is not intended to be run more than once and the global options for evaluation are set to FALSE. Once this notebook was completed, results were stored in RDS objects for further. 
...
pheno_df_list <- list(BRCA_pheno, GBM_pheno, OV_pheno, LUAD_pheno, UCEC_pheno, KIRC_pheno, HNSC_pheno, LGG_pheno, THCA_pheno, LUSC_pheno, PRAD_pheno, SKCM_pheno, COAD_pheno, STAD_pheno, BLCA_pheno, LIHC_pheno, CESC_pheno, KIRP_pheno, SARC_pheno, LAML_pheno, ESCA_pheno, PAAD_pheno, PCPG_pheno, READ_pheno, TGCT_pheno, THYM_pheno, KICH_pheno, ACC_pheno, MESO_pheno, UVM_pheno, DLBC_pheno, UCS_pheno, CHOL_pheno)

names(pheno_df_list) <- list("BRCA", "GBM", "OV", "LUAD", "UCEC", "KIRC", "HNSC", "LGG", "THCA", "LUSC", "PRAD", "SKCM", "COAD", "STAD", "BLCA", "LIHC", "CESC", "KIRP", "SARC", "LAML", "ESCA", "PAAD", "PCPG", "READ", "TGCT", "THYM", "KICH", "ACC", "MESO", "UVM", "DLBC", "UCS", "CHOL")

# save list of count dataframes for later
saveRDS(count_df_list, "output/tcga_counts_list.rds")
saveRDS(pheno_df_list, "output/tcga_metadata_list.rds")
...
```{r}
total_counts <- count_df_list %>% reduce(left_join, by = "gene")
saveRDS(total_counts, "output/flat_total_counts.rds")
```

```{r}
total_pheno <- bind_rows(pheno_df_list) %>%
  dplyr::rename(file_names = filename, file_id = id)
saveRDS(total_pheno, "output/flat_total_pheno.rds")
```

## Clinical annotation and metadata

Goals: Remove non-tumor data and split metastatic from primary tumors> Remove non-solid tumors.

#### Get extended pheno data

Read in data
```{r}
total_counts <- readRDS("output/flat_total_counts.rds")
total_pheno <- readRDS("output/flat_total_pheno.rds")
...
# combine downloaded sample data with kyle's table using submitter_id
...
pheno_df <- total_pheno %>% left_join(lookup_table)
```
...
```{r}
# review sample types
pheno_df %>% dplyr::select(sample_type) %>% 
  mutate(sample_type = as.factor(sample_type)) %>% 
  summary()

# remove blood cancers
pheno_data_filtered <- pheno_df %>%
  dplyr::filter(!project_id %in% c("TCGA-THYM", 
                                   "TCGA-LAML", 
                                   "TCGA-DLBC")) %>%
# keep only relevant tumor types
  dplyr::filter(sample_type %in% c("Primary Tumor", "Recurrent Tumor", "Metastatic")) %>%
  # review
  print()

# get included sample names
filteredsamples <- pheno_data_filtered %>% pull(sample)
```

#### Select count data

```{r}
# select columns based on membership in samplename list
gene_names <- total_counts %>%
  dplyr::select(gene)

counts_selected <- total_counts %>%
  dplyr::select(one_of(filteredsamples))

total_counts_selected <- bind_cols(gene_names, counts_selected)
```

## Save combined tables for later use

```{r}
write_csv(total_counts_selected, "output/selected.total.counts.csv")
write_csv(pheno_data_filtered, "output/filtered.total.phenodata.csv")
```

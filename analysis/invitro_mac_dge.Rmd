...
The purpose of this analysis is to find ranked differential gene expression sets for each of a number of experimentally derived macrophage states compared to monocytes. These gene sets will be used for our downstream analyses. The source for this method is:
...
The original experiment had two conditions. One (corresponding to the data we want) was done in cell culture with monocytes exposed to growth factor and a number of different cytokines to induce different activation states. They used those gene sets to investigate gene expression signatures from patients with COPD and asthma. The cell culture part was done on an Illumina HumanHT-12 v3.0 expression beadchip
The human part was done on Affymetrix chips

* The experiment overview is [here](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE46903).

* The record for the control group (ie M0-GM-CSF treated monocytes at 72hours) is [here](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSM1140443), and has more details about the experimental method.

* The research group posted expression matrices in which “The data were log2-transformed and normalized using quantile normalization with Partek Genomics Suite” An example expression matrix from the control group is [here](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?view=data&acc=GSM1140443&id=38019&db=GeoDb_blob97).

...
```{r}
hk_genes <- read_delim("data/HK_genes.tsv", 
                       delim = "\t", 
...
```{r}
# read in as matrix
ccle <- CePa::read.gct("data/CCLE_RNAseq_genes_counts_20180929.gct")
dim(ccle)

# remove hematopoietic cancer cell lines
ccle_non_heme <- ccle %>%
  as.data.frame() %>%
...
mac_eset <- readRDS("output/mac_eset_w_clusters.rds")
...
pheno_data %>% 
  ...
  write_csv("output/geo_id_adj_cluster_lookup_table.csv")
```

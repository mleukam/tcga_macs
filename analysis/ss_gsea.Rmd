...
boxplotter_up <- function(grouplist){
  df <- tidy_es_cleaned %>%
    ...
    mutate(cluster = str_sub(gene_set, 1, 2)) %>%
    mutate(cluster = ifelse(cluster == "T-", "T", cluster)) %>%
    mutate(cluster = fct_recode(cluster, "T" = "T", 
                                            "C1" = "C1", 
                                            "C2" = "C2", 
                                            "C3" = "C3", 
                                            "C4" = "C4", 
...
Combine plots into a single plot
```{r fig.height=16, fig.width=16}
combined_up_boxplot <- ggarrange(ACC, BLCA, BRCA, CESC, CHOL, COAD, ESCA, GBM, HNSC, KICH, KIRC, KIRP, LGG, LIHC, LUAD, LUSC, MESO, OV, PAAD, PCPG, PRAD, READ, SARC, SKCM_MET, SKCM_PRI, STAD, TGCT, THCA, UCS, UVM, 
                              ncol = 8, 
                              nrow = 4)
combined_up_boxplot
```
...
```{r}
# individual plots
my_comparisons <- list(c("C1", "T"))
KICH + stat_compare_means(comparisons = my_comparisons)
LGG + stat_compare_means(comparisons = my_comparisons)
THCA + stat_compare_means(comparisons = my_comparisons)
